...
async function fetchMovies(page = 1) {
  return get(`${TMDB}/discover/movie?${K}&with_original_language=ta&watch_region=IN&with_watch_monetization_types=flatrate&sort_by=primary_release_date.desc&page=${page}&language=en-US`);
}

async function fetchSeries(page = 1) {
  return get(`${TMDB}/discover/tv?${K}&with_original_language=ta&watch_region=IN&with_watch_monetization_types=flatrate&sort_by=first_air_date.desc&page=${page}&language=en-US`);
}

async function fetchDubbed(page = 1) {
  return get(`${TMDB}/discover/movie?${K}&watch_region=IN&with_watch_monetization_types=flatrate&with_original_language=hi%7Cte%7Cml%7Ckn%7Cen&sort_by=primary_release_date.desc&page=${page}&language=ta-IN`);
}
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginTop:"auto"}}>
          ...
          <div style={{display:"flex",gap:"4px",alignItems:"center"}}>
            {(item.platforms||[]).slice(0,3).map(p => {
              const m = PLATFORM_META[p];
              return m ? (
                <div key={p} title={p} style={{width:"7px",height:"7px",borderRadius:"50%",background:m.dot,boxShadow:`0 0 5px ${m.dot}66`}}/>
              ) : null;
            })}
            {(item.platforms||[]).length > 0 && (
              <span style={{fontSize:"10px",color:C.muted,marginLeft:"3px"}}>{item.platforms[0]}</span>
            )}
          </div>
...
export default function App() {
  ...
  const anyFilter = filters.platform!=="All"||filters.langType!=="All"||filters.mediaType!=="All"||search;
  ...
  return (
    <div style={{minHeight:"100vh",background:C.base,color:C.body,fontFamily:"'DM Sans','Segoe UI',sans-serif"}}>
      ...
      {/* ── HERO ── */}
      {!anyFilter && !loading && (
        <div style={{maxWidth:"1400px",margin:"0 auto",padding:isMobile?`22px ${px} 0`:`36px ${px} 0`}}>
          <h1 style={{
            fontFamily:"'Fraunces',Georgia,serif",
            fontSize:isMobile?"clamp(22px,7vw,28px)":"clamp(26px,3.5vw,38px)",
            fontWeight:"900",color:C.head,lineHeight:"1.1",letterSpacing:"-0.5px",marginBottom:"6px",
          }}>
            Tamil OTT Catalogue<br/>
            <em style={{color:C.gold,fontStyle:"italic",fontWeight:"700",fontSize:"0.75em"}}>live from TMDb · India</em>
          </h1>
          <p style={{color:C.muted,fontSize:"13px",marginBottom:"24px",maxWidth:"420px",lineHeight:"1.65"}}>
            Browse original Tamil releases and dubbed Tamil content streaming now on Netflix, Prime Video, JioHotstar, ZEE5, Sun NXT, SonyLIV and more.
          </p>
          <div style={{height:"1px",background:`linear-gradient(90deg,${C.gold}44,${C.border},transparent)`,marginBottom:"22px"}}/>
        </div>
      )}
      ...
      <main style={{maxWidth:"1400px",margin:"0 auto",padding:anyFilter?`18px ${px} 60px`:`0 ${px} 60px`}}>
        {error ? (
          ...
          <div style={{textAlign:"center",padding:"80px 20px"}}>
            ...
            <p style={{color:C.muted,fontSize:"15px",fontFamily:"'Fraunces',Georgia,serif"}}>No titles match your filters</p>
            <button onClick={()=>{setSearch("");setFilters({platform:"All",langType:"All",mediaType:"All",sortBy:"date"});}} style={{marginTop:"14px",background:"transparent",border:`1px solid ${C.border}`,color:C.muted,padding:"8px 18px",borderRadius:"6px",cursor:"pointer",fontSize:"12px",fontFamily:"inherit"}}>Clear all filters</button>
          </div>
        ) : (
          <>
            <div style={{display:"grid",gridTemplateColumns:gridCols,gap:isMobile?"12px":"16px"}}>
            ...
            {/* Load more */}
            {hasMore && !filters.platform && filters.langType !== "Dubbed Tamil" && (
